...
//数値計算
var Length = 10;//ダミーを含む計算領域の長さ（ｘ軸,[m]）
var Length0 = 10;//ダミーを含まない計算領域の長さ
...
function webMain() 
{
  ...
  form2.pulseWidth.value = pulseWidth;
  form2.waveVel.value = waveVel;
  form2.scale.value = rect.scale;
  form2.pulseX0.value = pulseX0; 
  initData();
  ...
  var animate = function()
  {
    ...
    if(flagStart)
    {
      elapseTime += deltaT0;//数値計算上の経過時間（実経過時間ではない）
...
function initData()
{
  ...
  pulseX0 = parseFloat(form2.pulseX0.value);
  pulseWidth = parseFloat(form2.pulseWidth.value);
  waveVel = parseFloat(form2.waveVel.value);
  rect.scale = parseFloat(form2.scale.value);
  NX0 = parseFloat(form2.nMesh.value);//空間分割数
  
  DX = Length0 / NX0;//格子間隔
  var n_pWidth = pulseWidth / DX;//パルス幅の格子数
  var n_pX0 = pulseX0 / DX;
  ...
  for (i = 0; i <= NX; i++)
  {
    ...
    if(mode == "REC")
    {
      if(i > NX/2 - n_pWidth/2 + n_pX0 && i < NX/2 + n_pWidth/2 + n_pX0)  
        pos[i]  = 1;
    }
    else
    {
      if(i > (NX/2 - n_pWidth) + n_pX0 && i < (NX/2 + n_pWidth)  + n_pX0)  
        pos[i]  = 0.5 * Math.cos(Math.PI*(i - NX/2 - n_pX0)/n_pWidth) + 0.5;
    }
  }
}
...
function display()
{
  ...
  for(i = 0; i <= 3; i++)
  {
    drawLine(rect.left0.x + i * 0.5 * rect.size.x * scale.x, rect.left0.y,
		rect.left0.x + i * 0.5 * rect.size.x * scale.x, rect.left0.y + h0 * scale.y, 1, "light_blue");//"light_gray");
  }
...
{
  var i;
  var D2 = DX * DX;//(sizeX0=sizeY0,NX0=NY0, DX = DYであること）
  var mu = mu0;//減衰係数
  var muMax = 5.0;
  ...
  for(i = 0; i <= NX; i++)
  {
    if(boundary == "B_FIXED")
    {
      ...
      if(i == NX) { pos[NX] = 0; continue; }
    }
    else if(boundary == "B_FREE")
    {
      if(i == 0)  { pos[0] = pos[1]; continue; }
      if(i == NX) { pos[NX] = pos[NX-1]; continue; }
    }
