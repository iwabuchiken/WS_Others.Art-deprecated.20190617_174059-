...
var waveVel = 1;//伝搬速度
var mu0 = 0.0;//粘性抵抗
var sourceY0 = 0.0;//分布中心のx座標
var sourceY0 = 0.0;//分布中心のy座標
var vel = []; //z軸方向の速度
var pos = []; //変位量

var mode = "CYLINDER";//単発，連続
...
{  
  ...
  NY0 = NX0;
  sourceX0 = parseFloat(form2.X0.value);//分布の中心位置
  sourceY0 = parseFloat(form2.Y0.value);
  waveVel = parseFloat(form2.waveVel.value);//伝搬速度
  ...
  //初期分布
  var dia = parseFloat(form2.dia.value);
  var dist=0;
  var dx = sizeX / NX;
  var dy = sizeY / NY;
  var x = y = 0;
  var x0 = sizeX / 2 + sourceX0;
  var y0 = sizeY / 2 + sourceY0;
  for(var j = 0; j <= NY; j++)
    for(var i = 0; i <= NX; i++) 
    {
      x = i * dx - x0; y = j * dy - y0;
      ...
      if(mode == "CYLINDER")
      {
        if(dist < dia/2) pos[i][j] = amp0;
      }
      else
      {
        if(dist < dia) pos[i][j] = amp0 * 0.5 * (Math.cos(Math.PI*dist/dia) + 1);
      }
    } 
...
{ 
  ...
  for(j = nDummy; j <= NY0 + nDummy; j++){
    for(i = nDummy; i <= NX0 + nDummy; i++){
      //np = (j - nDummy) * (NX0 + 1) + i - nDummy;
      //w_object.data[np] = pos[i][j];
