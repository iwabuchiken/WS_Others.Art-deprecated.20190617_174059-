...
function initData()
{
  ...
  var nMeshY1 = (NY - nObsWY) / 2;//障害物下端位置
  var nMeshY2 = (NY + nObsWY) / 2;//障害物上端位置
  ...
  for(j = 0; j <= NY; j++)
  {
	for(i = 0; i <= NX; i++)
    {
	  ...
	  if(i > nMeshX1 && i < nMeshX2 && j == nMeshY1) type[i][j] = "OBS_BOTTOM";//障害物上端
	  if(i > nMeshX1 && i < nMeshX2 && j > nMeshY1 && j < nMeshY2) type[i][j] = "OBSTACLE";//障害物内部
	  //コーナー
	  if(i == nMeshX1 && j == nMeshY1) type[i][j] = "OBS_LL";
	  if(i == nMeshX1 && j == nMeshY2) type[i][j] = "OBS_UL";
	  if(i == nMeshX2 && j == nMeshY1) type[i][j] = "OBS_LR";
	  if(i == nMeshX2 && j == nMeshY2) type[i][j] = "OBS_UR";
	}
  }
  ...
  for(j = 0; j <= NY; j++)  
	...
	for (i = 0; i <= NX; i++)
	{
      ...
      else Psi[i][j] = rect.size.y / 2;
	  //渦度
	  Omega[i][j] = 0.0;
      //速度
	  if(type[i][j] == "INLET" || type[i][j] == "OUTLET" || type[i][j] == "INSIDE") VelX[i][j] = 1.0;
      else if(type[i][j] == "BOTTOM" || type[i][j] == "TOP") VelX[i][j] = ideal; 
	  else VelX[i][j] = 0.0;
      VelY[i][j] = 0.0;
      //CIP法で使用する微分値
      gx[i][j] = 0.0;
...
//if(i==40) console.log(" j = " + j + " range = " + range0 + " omega = " + Omega[i][j] + " data = " + object.data[k]);
      }
      else  
      {
...
{
  var i, j;
  var iteration =100;//最大繰り返し回数(Poisson方程式）
  ...
  for (i = 0; i <= NX; i++)
	for (j = 0; j <= NY; j++) 
	{
	  ...
	  else if(type[i][j] == "TOP") Omega[i][j] = -2.0 *( Psi[i][NY-1] - rect.size.y + ideal * DY) / DY2;
	  else if(type[i][j] == "BOTTOM") Omega[i][j] = -2.0 * (Psi[i][1] - ideal * DY)/ DY2;
	  else if(type[i][j] == "OBS_LEFT") Omega[i][j] = -2.0 * (Psi[i-1][j] - psi_obj) / DX2;
	  else if(type[i][j] == "OBS_RIGHT") Omega[i][j] = -2.0 * (Psi[i+1][j] - psi_obj) / DX2;
	  else if(type[i][j] == "OBS_TOP") Omega[i][j] = -2.0 * (Psi[i][j+1] - psi_obj)/ DY2;
	  else if(type[i][j] == "OBS_UL") Omega[i][j] = -2.0 * (Psi[i-1][j+1] - psi_obj) / DD2;
	  else if(type[i][j] == "OBS_UR") Omega[i][j] = -2.0 * (Psi[i+1][j+1] - psi_obj)/ DD2;
	  else if(type[i][j] == "OBS_LL") Omega[i][j] = -2.0 * (Psi[i-1][j-1] - psi_obj) / DD2;
	  else if(type[i][j] == "OBS_LR") Omega[i][j] = -2.0 * (Psi[i+1][j-1] - psi_obj)/ DD2;
    }
