...
var flagUniformWing = true;
var flagDoublet  = false;
...
var xi0 = -0.1;//radCylinderで割った値
var eta0 = 0.2;//radCylinderで割った値
var maxVelocity = 2.5;//圧力計算に必要
var arrowScale = 0.05;
var arrowWidth = 1;
...
{  
  //ポテンシャル流れの計算パラメータ
  flagUniformWing = form2.Uniform.checked ;
  ...
  var maxVel2 = maxVelocity * maxVelocity;
  ...
  var z = new Vector3();
  var rr = 0, c1, c2, r2, r4, cc, ss;
  var rad0 = 0.001;
  var radC2 = radCylinder * radCylinder;//円柱半径の2乗
  var ang = alpha * DEG_TO_RAD;//迎え角
  ...
  for (i = 0; i <= rect.nMesh; i++)
  {
    ...
    for (j = 0; j <= rect.nMesh; j++)
    {
      ...
      cc = Math.cos(ang); ss = Math.sin(ang);
      if(flagUniformWing) 
      {//x軸に水平な流れで計算
	    Phi[i][j] = flowVelocity * (z.x * cc + z.y * ss);
		Psi[i][j] = flowVelocity * (z.y * cc - z.x * ss);
		VelX[i][j] = flowVelocity * cc;
		VelY[i][j] = flowVelocity * ss;
		//円柱
        if (z.x == 0 && z.y == 0)
        {//対数的特異点
        ...
        r2 = mag2(z);//2乗距離
        r4 = r2 * r2;
        Phi[i][j] += flowVelocity * radC2 *(z.x * cc + z.y * ss) / r2;
        Psi[i][j] += flowVelocity * radC2 *(z.x * ss - z.y * cc) / r2;
		c1 = flowVelocity * radC2 * ( z.x * z.x - z.y * z.y ) / r4;
		c2 = 2.0 * flowVelocity * radC2 * z.x * z.y / r4;
		VelX[i][j] -= c1 * cc + c2 * ss;
		VelY[i][j] += c1 * ss - c2 * cc;
      }
...
function display()
{
  ...
  xi0 = parseFloat(form2.xi0.value);
  eta0 = parseFloat(form2.eta0.value);
  ...
  function drawWing()
  {
    var theta0 = 2 * Math.PI / 30;
    for(var i = 0; i < 30; i++)
    {
      var pos1 = new Vector3();
      pos1.x = radCylinder * Math.cos(i * theta0);
      pos1.y = radCylinder * Math.sin(i * theta0);
      var pos2 = new Vector3();
      pos2.x = radCylinder * Math.cos((i+1) * theta0);
      pos2.y = radCylinder * Math.sin((i+1) * theta0);
      //pos1 = mappingWing(pos1); pos2 = mappingWing(pos2);
      pos1 = mapping(pos1); pos2 = mapping(pos2);
      pos1 = mul(pos1, scale); pos2 = mul(pos2, scale);
      drawLine(pos1.x, pos1.y, pos2.x, pos2.y, 2, "black");
    }
  }
}
...
{
  var rad = mag(pos);//計算空間における原点からの距離
  var theta = Math.atan2(pos.y, pos.x);
  var cc = radCylinder * radCylinder;
  //計算平面における拡大・平行移動
  var RDR =  Math.sqrt((1.0 - xi0)*(1.0 - xi0) + eta0 * eta0);//R'/R
  var A = RDR * rad * Math.cos(theta) + xi0 * radCylinder;
  var B = RDR * rad * Math.sin(theta) + eta0 * radCylinder;
  var AB = A * A + B * B;
  var C =   cc * A / AB;
  var D = - cc * B / AB;
  //計算平面から物理平面への座標変換
...
function drawParticle(dt)
{
  ...
  for(k = 0; k < numMaxP; k++)
  {
	...
	if(pos.x >= -0.1 && pos.x < rect.size && pos.y >= -0.1 && pos.y < rect.size ) 
    { 
...
{
  pa[k].pos.x = 0.1;//左端(left0)からの位置
  //pa[k].pos.y = rect.size * getRandom(0, 1);
  pa[k].pos.y = rect.size * getRandom(-0.2, 2);
  
  var pos0 = new Vector3(rect.size/2, rect.size/2, 0);
  if(flagMapping){
    pa[k].pos.x -= pos0.x;
    pa[k].pos.y -= pos0.y;
    pa[k].pos.rotZ_deg(alpha);
    pa[k].pos.x += pos0.x;
    pa[k].pos.y += pos0.y;
  }
}
