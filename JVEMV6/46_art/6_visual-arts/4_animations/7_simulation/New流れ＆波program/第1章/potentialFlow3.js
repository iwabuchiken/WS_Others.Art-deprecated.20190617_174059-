...
{  
  ...
  flagDoublet = form2.Doublet.checked ;
  ...
  {  
    for (i = 0; i <= rect.nMesh; i++)
      Phi[i][j] = Psi[i][j] = VelX[i][j] = VelY[i][j] = Press[i][j] = 0;
  }
  ...
  for (i = 0; i <= rect.nMesh; i++)
  {
    ...
    for (j = 0; j <= rect.nMesh; j++)
    {
      ...
      if (flagDoublet)//湧き出し＋吸い込み
	  {
        ...
        r2 = mag2(z);//2乗距離
        r4 = r2 * r2;
        Phi[i][j] += flowVelocity * radC2 *(Math.cos(ang) * z.x + Math.sin(ang) * z.y) / r2;
        Psi[i][j] += flowVelocity * radC2 *(Math.sin(ang) * z.x - Math.cos(ang) * z.y) / r2;
		c1 = flowVelocity * radC2 * ( z.x * z.x - z.y * z.y ) / r4;
		c2 = 2.0 * flowVelocity * radC2 * z.x * z.y / r4;
		VelX[i][j] -= c1 * Math.cos(ang) + c2 * Math.sin(ang);
		VelY[i][j] += c1 * Math.sin(ang) - c2 * Math.cos(ang);
      }
...
function display()
{
  ...
  if(flagCylinder)//円柱表示
    drawCircle(0, 0, 2*radCylinder * scale.x, 2*radCylinder * scale.y, true, "light_gray", 0) ;
...
function drawPressure()
{
  ...
  for (j = 0; j < rect.nMesh; j++)
  {
    for (i = 0; i < rect.nMesh; i++)
    {
	  ...
	  for(k = 0; k < 4; k++) {//矩形領域中心を原点とした座標
       pos[k].x -= pos0.x; pos[k].y -= pos0.y; 
      }
...
//      dataP[2*K+1] = rect.left0.y + pa[K].pos.y * scale.y; 
      dataP.push(rect.left0.x + pa[K].pos.x * scale.x); 
      dataP.push(rect.left0.y + pa[K].pos.y * scale.y); 
      K++;
    }
    else{
  ...
  drawPoints(dataP, sizeP, typeP, "black");//swgShape2D.js
...
{
  pa[k].pos.x = 0.;//0.01;//左端(left0)からの位置
