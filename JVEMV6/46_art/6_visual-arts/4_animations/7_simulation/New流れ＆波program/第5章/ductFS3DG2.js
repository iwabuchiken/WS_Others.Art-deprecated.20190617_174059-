...
var flagReset = false;
//表示オブジェクト
var object, obstacle1, obstacle2;
...
function Rect()
{
  ...
  this.obs_nY10 = 30;//下端から障害物中心までの距離(格子間隔の整数倍）
  this.obs_nX20 = 15;//左端から障害物中心までの距離(格子間隔の整数倍）
  this.obs_nY20 = 50;//下端から障害物中心までの距離(格子間隔の整数倍）
  this.obs_nWX = 2;//障害物の厚さ(ｘ方向,格子間隔の整数倍)
  this.obs_nWY = 4; //障害物の幅(ｙ方向,格子間隔の整数倍)
}
var rect = new Rect();
var NX, NY, DX, DY;
...
function initObject()
{
  ...
  obstacle1 = new Rigid();
  obstacle1.kind = "CUBE";
  obstacle1.vSize = new Vector3(rect.obs_nWX * DX, rect.obs_nWY * DY, 0.1);
  obstacle1.vPos = new Vector3(rect.obs_nX10 * DX - rect.size.x/2, rect.obs_nY10 * DY - rect.size.y/2, 0);
  obstacle1.flagDebug = flagDebug;
  //障害物2
  obstacle2 = new Rigid();
  obstacle2.kind = "CUBE";
  obstacle2.vSize = new Vector3(rect.obs_nWX * DX, rect.obs_nWY * DY, 0.1);
  obstacle2.vPos = new Vector3(rect.obs_nX20 * DX - rect.size.x/2, rect.obs_nY20 * DY - rect.size.y/2, 0);
  obstacle2.flagDebug = flagDebug;
...
//if(i==40) console.log(" j = " + j + " range = " + range0 + " omega = " + Omg[i][j] + " data = " + object.data[k]);
      }
      else  
      {
  ...
  n = obstacle1.initVertexBuffers(gl);
  obstacle1.draw(gl, n);
  //障害物2
  n = obstacle2.initVertexBuffers(gl);
  obstacle2.draw(gl, n);
...
{
  object.flagDebug = obstacle1.flagDebug = obstacle2.flagDebug = flagDebug = !flagDebug ;
  display(); 
}
