...
//表示オブジェクト
var object, obstacle;
var adjustH = 0.01;//表示上の高さ調整var texNo = 0;
var texNo = 0;
...
var obsPos = new Vector3(0.2, 0.5, 0);//ダクト左下を原点とした位置
var obsVel = new Vector3(1, 0, 0);
var obsRadius = 0.3;//円運動時の半径
var moveMode = 0;//直線
...
　//カメラ・インスタンスを作成
  camera = new Camera();
  camera.phi = 0; 
  camera.theta = 90.01;//完全に真上では表示されない
  camera.cnt[2] = 0;
  camera.dist = 4;
  camera.fovy = 15;
...
//if(i == NX/2 && j > 10 && j < 20) console.log("CCC px = " + g_posX[i][j] + " py = " +g_posY[i][j]);
    }
...
function initObject()
{
  ...
  obstacle.flagTexture = false;//true;
}
...
function display()
{
  ...
  for(j = 0; j <= NY; j++)
    for(i = 0; i <= NX; i++)
    {
//if(i == NX/2 && j > 10 && j < 20) console.log("i = " + i + " j = "+j + " x = " + g_posX[i][j]+ " y = " + g_posY[i][j]);
      k = i + j * (NX+ 1);
  ...
  //物理量表示オブジェクト
  if(object.flagTexture)
  {
    object.diffuse = [ 0.9, 0.9, 0.9, 1.0] ;
    object.ambient = [ 0.5, 0.5, 0.5, 1.0];
  }
  else
  {
...
//  gl.uniform1i(colorLoc, 2);
  obstacle.vPos.x = obsPos.x - rect.size.x/2;
  obstacle.vPos.y = obsPos.y - rect.size.y/2;
...
function renewObsPos()
{
  if(moveMode == 0)//直線モード
  { 
    if(obsPos.x >= 0.9) obsVel.x = - obsSpeed;
    if(obsPos.x <= 0.1) obsVel.x =   obsSpeed;
    obsVel.y = 0;
  }
  else//円運動
  {//初期位置は左側
