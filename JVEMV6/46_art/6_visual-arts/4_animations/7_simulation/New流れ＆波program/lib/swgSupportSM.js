...
Camera.prototype.pickObject = function(vWorldPos)
{
  ...
  var min = 1000;
  var pickNo;
  for(i = 0; i < springMass.numPoint; i++)
  {
    vDir = sub(springMass.point[i].vPos, vCameraPos);
    s = dot(unitVector, vDir);
    vPoint = mul(unitVector, s);
    vPoint.add(vCameraPos);//交点
    dist[i] = distance(vPoint, springMass.point[i].vPos);
...
　　  if(x > rect.left && x < rect.left + wid && y > rect.top && y < rect.top + wid)//canvasの左上
  ...
  {//質点番号を取得しその質点を操作
    ...
    canvas.onmousedown = function(ev)
    {
      ...
      springMass.point[pickNo].flagFixed = true;
      vOld = sub(vCameraPos , mul(dot(vNormal, sub(vCameraPos, springMass.point[pickNo].vPos))
           / dot(vNormal, vDirMouse), vDirMouse));//質点が存在するplaneとvDirMouseの交点
      }

      else {//剛体が選択されたとき
        springMass.rigid2.flagFixed = true;
        springMass.rigid2.state = "RESTING";
        vOld = sub(vCameraPos , mul(dot(vNormal, sub(vCameraPos, springMass.rigid2.vPos))
		 	 / dot(vNormal, vDirMouse), vDirMouse));//質点が存在するplaneとvDirMouseの交点
      }
    }
    ...
    canvas.onmouseup = function(ev)
    {
      if(m_rigidNo == 0) springMass.point[pickNo].flagFixed = false;
      else{ springMass.rigid2.flagFixed = false; 
      springMass.rigid2.state = "FREE"; }
      flagMove = false; 
      flagMouse = false;
    }
    ...
    　if(x < rect.left || x > rect.right || y < rect.top || y > rect.bottom)
      ...
      if(m_rigidNo == 0){//質点
        vNew = sub(vCameraPos , mul(dot(vNormal, sub(vCameraPos, springMass.point[pickNo].vPos))
			 / dot(vNormal, vDirMouse), vDirMouse));//質点が存在するplaneとvDirMouseの交点
        var vMove = sub(vNew, vOld);
        if(springMass.point[pickNo].flagFixed == true) springMass.point[pickNo].vPos.add(vMove);
      }      
      else {//剛体が選択されたとき
        vNew = sub(vCameraPos , mul(dot(vNormal, sub(vCameraPos, springMass.rigid2.vPos))
	   		 / dot(vNormal, vDirMouse), vDirMouse));//質点が存在するplaneとvDirMouseの交点
        var vMove = sub(vNew, vOld);
        if(springMass.rigid2.flagFixed == true) springMass.rigid2.vPos.add(vMove);
      }
      vOld = vNew;
    }
  }
