...
//console.log("numPoint = " + this.numPoint + "  n0 = " + n0);
		...
		this.point[this.numPoint].vSize = new Vector3(2.0*rad, 2.0*rad, 2.0*rad);
		this.point[this.numPoint].vPos0.x = this.point[this.numPoint].vPos.x = this.point[n0].vPos.x + len * Math.cos(this.beta) * Math.cos(gamma * j + this.alpha * (k -1.0)) * (1.0 + getRandom(-va, va));
		this.point[this.numPoint].vPos0.y = this.point[this.numPoint].vPos.y = this.point[n0].vPos.y + len * Math.cos(this.beta) * Math.sin(gamma * j + this.alpha * (k -1.0)) * (1.0 + getRandom(-va, va));
  ...
  rad = this.radiusBranch0 * this.rate;
  this.beta *= this.rate;//M_PI / 15.0;//水平面との角度
  gamma = 2.0 * Math.PI / this.numBranch0;//取り付け位置（x軸からの経度）
	...
	for(j = 0; j < this.numBranch0; j++)
	{
      for(k = 0; k < this.numBranch; k++)
	  {
		for(m = 0; m < this.numBranch; m++)
		{
          ...
          this.point[this.numPoint].kind = "SPHERE";
		  this.point[this.numPoint].vSize = new Vector3(2.0*rad, 2.0*rad, 2.0*rad);
		  this.point[this.numPoint].vPos0.x = this.point[this.numPoint].vPos.x = this.point[n0].vPos0.x + len * Math.cos(this.beta) * Math.cos(gamma * j + this.alpha * (k+m -2.0) ) * (1.0 + getRandom(-va, va));
		  this.point[this.numPoint].vPos0.y = this.point[this.numPoint].vPos.y = this.point[n0].vPos0.y + len * Math.cos(this.beta) * Math.sin(gamma * j + this.alpha * (k+m -2.0)) * (1.0 + getRandom(-va, va));
