...
{  
  ...
  for(j = 0; j <= nMesh; j++)
    for(i = 0; i <= nMesh; i++)
    {
	  ...
	  y = j * delta - 0.5*size;
	  //y軸に平行な中心導体
	  if(x >= x1 ) g =  (x - (x1 + width0/2)) / b;
	  else         g = -(x - (x1 - width0/2)) / b;
	  D[k] = (diffCoef1 - diffCoef2) / (1 + Math.exp(g)) + diffCoef2;
	  //x軸に平行な中心導体
	  if(y >= y1 ) g =  (y - (y1 + width0/2)) / b;
	  else         g = -(y - (y1 - width0/2)) / b;	  
	  D[k] += (diffCoef1 - diffCoef2) / (1 + Math.exp(g)) + diffCoef2;
	  if(D[k] > diffCoef1) D[k] = diffCoef1;//中心導体が重なる部分
    }
}
...
function calculate()
{
  ...
  for(j = 0; j <= nMesh; j++)
    for(i = 0; i <= nMesh; i++)
    {
	  k = i + j * (nMesh + 1);
	  f0[k] = object.data[k];
//if(j == 10 && i < 10) console.log("i = " + i + " f = " + f0[k]);
    }
