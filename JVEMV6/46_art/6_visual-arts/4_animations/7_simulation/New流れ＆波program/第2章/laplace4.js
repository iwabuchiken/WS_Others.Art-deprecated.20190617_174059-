...
var nMeshX0, nMeshX1, nMeshX2, nObsWX, nObsWY;
...
function init()
{
  ...
  nMeshX0 = Math.floor((rect.obs_x0+eps) / rect.delta.x);//0.2が0.19999999999となることあるため
  nMeshX1 = Math.floor((rect.obs_left+eps) / rect.delta.x);
  nMeshX2 = Math.floor((rect.obs_left + rect.obs_widthX + eps) / rect.delta.x);
  nObsWX = nMeshX2 - nMeshX1;  var i, j;
  ...
  nObsWY = Math.floor((rect.obs_widthY+eps) / rect.delta.y);
...
{  
  ...
  for(j = 0; j <= rect.nMeshY; j++)
  {
	...
	{
	  k = i + j * (rect.nMeshX+1);
	  if(type[i][j] == "BOTTOM" || type[i][j] == "OBS_LEFT" || type[i][j] == "OBS_TOP" || type[i][j] == "OBS_RIGHT") Psi[i][j] = 0.0;
	  else if(type[i][j] == "TOP")  Psi[i][j] = rect.size.y;//一様流れの流速を1とする
	  else
	  {//内点および入口・出口は線形補間
	    if(i < nMeshX1 || i > nMeshX2)   Psi[i][j] = j * rect.delta.y;
	    else  Psi[i][j] = rect.size.y * (j - nObsWY) / (rect.nMeshY - nObsWY);
	  }
	}
  }
...
//console.log(" cnt = " + cnt + " pp = " + pp + " error = " + error);
...
console.log("AAA cnt = " + cnt + " maxError = "+ maxError + " delta.x = " + rect.delta.x);
  ...
  for (j = 1; j < rect.nMeshY; j++)
	...
	{ 
	  ...
	  //境界上
	  if(type[i][j] == "INLET" || type[i][j] == "OUTLET") { VelX[i][j] = 1; VelY[i][j] = 0; }
	  if(type[i][j] == "BOTTOM" || type[i][j] == "TOP") { VelY[i][j] = 0; }
	  if(type[i][j] == "OBS_LEFT" || type[i][j] == "OBS_RIGHT") { VelX[i][j] = 0; }
      if(type[i][j] == "CORNER_UL" || type[i][j] == "CORNER_UR") {VelX[i][j]= 0; VelY[i][j] = 0;}	  
