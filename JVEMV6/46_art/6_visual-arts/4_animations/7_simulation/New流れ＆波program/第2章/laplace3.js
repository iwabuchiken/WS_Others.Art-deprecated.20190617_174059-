...
{  
  ...
  while (cnt < iteration)
  {
    ...
    for (j = 1; j < rect.nMeshY; j++)
    {
	  ...
	  {
//console.log("j = " + j + " i = " + i+ " type =" + type[i][j]); 
  ...
  while (cnt < iteration)
  {
    ...
    for(j = 0; j <= rect.nMeshY; j++)
    {
      for(i = 1; i < rect.nMeshX; i++)
      {
		...
		else if(type[i][j] == "OBS_UL")  Phi[i][j] = Phi[i-1][j+1];
		else if(type[i][j] == "OBS_UR")  Phi[i][j] = Phi[i+1][j+1];
	  }
    }
